...
Telegram-бот для таро-раскладов с интерпретацией через DeepSeek API. Опционально — второй бот для гадания по рунам (`TELEGRAM_BOT_TOKEN_RUNES`) в том же процессе.
...
### Руны (если задан токен)
- **Одна руна** / **Три руны** / **Руна дня**
- **Параметры** — только авторассылка руны дня (без перевёрнутых)
- Картинки: овал + Unicode-глиф в памяти (`fonts/NotoSansRunic-Regular.ttf`); руна «Один» — пустой овал
- Толкование через тот же DeepSeek

Пользователи общие (`users`); настройки и история раскладов — раздельно по ботам.
...
| `TELEGRAM_BOT_TOKEN_RUNES` | нет | Токен бота рун (второй poller+scheduler при наличии) |
